...
async def test_upload_import_stages_amr_audio_file(
    api_client: AsyncClient,
) -> None:
    async with api_client as client:
        response = await client.post(
            "/api/v1/imports/uploads",
            data={
                "title": "Uploaded AMR Story",
                "content_type": "Audiobook",
                "requested_by_user_slug": "dale",
            },
            files={"media_file": ("uploaded-story.amr", b"fake amr bytes", "audio/amr")},
        )

    assert response.status_code == 201
    payload = response.json()
    assert payload["source_type"] == "browser_upload"
    assert payload["source_path"].endswith(".amr")
    assert payload["related_job_id"] is not None
    assert Path(payload["source_path"]).exists()
...
async def test_yoto_remote_payload_preview_generates_stream_content(
    api_client: AsyncClient,
    db_session: Session,
) -> None:
    item = LibraryItem(
    ...
    async with api_client as client:
        response = await client.get(f"/api/v1/yoto/playlists/{draft.id}/remote-payload")

    assert response.status_code == 200
    payload = response.json()
    assert payload["can_create_live"] is True
    assert payload["warnings"] == []
    assert payload["payload"]["content"]["chapters"][0]["tracks"][0]["trackUrl"] == "https://example.com/live.mp3"


async def test_create_live_yoto_playlist_updates_draft_and_linked_cards(
    api_client: AsyncClient,
    db_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    card = PhysicalCard(
        card_code="LIVE001",
        display_name="Blank Test Card",
        card_kind="generic_blank",
        current_library_item_id=item.id,
        status="upload_queued",
    )
    credential = YotoCredentialState(
        account_label="Household Yoto",
        status="connected_tested",
        token_storage_ref="k8s-secret:test-namespace:test-secret:yoto-credential-live.json",
    ...
    db_session.add_all([card, credential, draft])
    db_session.commit()
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["remote_card_id"] == "31yYU"
    assert payload["playlist"]["status"] == "remote_created"

    refreshed_draft = db_session.get(YotoPlaylistDraft, draft.id)
    refreshed_item = db_session.get(LibraryItem, item.id)
    refreshed_card = db_session.get(PhysicalCard, card.id)
    version = db_session.scalar(
        select(YotoPlaylistVersion)
        .where(YotoPlaylistVersion.playlist_draft_id == draft.id)
        .where(YotoPlaylistVersion.source_event == "yoto_remote_created")
    )
    event = db_session.scalar(
        select(VersionEvent)
        .where(VersionEvent.entity_type == "library_item")
        .where(VersionEvent.entity_id == item.id)
        .where(VersionEvent.event_type == "yoto_remote_created")
    )
    assert refreshed_draft is not None
    assert refreshed_draft.remote_playlist_id == "31yYU"
    assert refreshed_item is not None
    assert refreshed_item.status == "yoto_remote_created"
    assert refreshed_card is not None
    assert refreshed_card.ready_to_link_in_app is True
    assert refreshed_card.status == "ready_to_link"
    assert refreshed_card.notes is not None
    assert "31yYU" in refreshed_card.notes
    assert version is not None
    assert event is not None
...
async def test_create_live_yoto_playlist_uploads_local_audio_assets(
    ...
    track = PlaylistTrack(
        library_item_id=item.id,
        title="Chapter One",
        source_path=str(tmp_path / "chapter-one-source.mp3"),
        track_number=1,
        duration_seconds=61,
        is_stream=False,
    )
    ...
    processed_path = tmp_path / "chapter-one-processed.mp3"
    processed_path.write_bytes(b"processed-audio-bytes")
    asset = ProcessedAsset(
        library_item_id=item.id,
        playlist_track_id=track.id,
        source_path=track.source_path or str(processed_path),
    ...
    async def fake_call_yoto_api(
        *,
        ...
        api_calls.append((method, relative_url, json_body))
        assert api_base_url == "https://api.yotoplay.com"
        assert access_token == "test-access-token"
        if method == "GET" and relative_url.startswith("/media/transcode/audio/uploadUrl?"):
            return 200, {"uploadId": "upload-123", "uploadUrl": "https://signed.example.test/upload-123"}
        if method == "GET" and relative_url == "/media/upload/upload-123/transcoded?loudnorm=false":
            return 200, {"transcodedSha256": "abc123sha"}
        if method == "POST" and relative_url == "/content":
            assert isinstance(json_body, dict)
            chapters = json_body["content"]["chapters"]
        ...
        if method == "GET" and relative_url == "/content/REMOTE123":
            return 200, {"card": {"cardId": "REMOTE123", "title": "Uploaded Draft"}}
        raise AssertionError(f"Unexpected Yoto API call: {method} {relative_url}")

    monkeypatch.setattr(yoto_routes, "load_tokens_from_secret", fake_load_tokens_from_secret)
    monkeypatch.setattr(yoto_routes, "_upload_file_to_yoto_signed_url", fake_upload_file_to_yoto_signed_url)
    monkeypatch.setattr(yoto_routes, "_call_yoto_api", fake_call_yoto_api)
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["remote_card_id"] == "REMOTE123"
    assert upload_calls == [("https://signed.example.test/upload-123", str(processed_path))]
    assert api_calls[0][0] == "GET"
    assert api_calls[1] == ("GET", "/media/upload/upload-123/transcoded?loudnorm=false", None)
    assert api_calls[2] == ("POST", "/content", api_calls[2][2])
    assert api_calls[3] == ("GET", "/content/REMOTE123", None)
...
async def test_create_live_yoto_playlist_rejects_raw_amr_without_processed_asset(
    ...
    db_session.add(item)
    db_session.flush()
    source_path = tmp_path / "voice-note.amr"
    source_path.write_bytes(b"fake-amr")
    track = PlaylistTrack(
        library_item_id=item.id,
        title="Voice Note",
    ...
    db_session.add_all([track, credential, draft])
    db_session.commit()
